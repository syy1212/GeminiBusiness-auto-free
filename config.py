...
class Config:
    def __init__(self):
        ...
        if not os.path.exists(dotenv_path):
            env_example_path = os.path.join(application_path, ".env.example")
            if os.path.exists(env_example_path):
                error_msg = (
                    f"\n配置文件 .env 未找到！\n"
                    f"请按以下步骤创建配置文件：\n"
                    f"1. 将 .env.example 复制为 .env\n"
                    f"2. 编辑 .env 文件，填写您的实际配置\n"
                    f"   .env.example 路径: {env_example_path}\n"
                )
            else:
                error_msg = get_translation("file_not_exists", path=dotenv_path)
